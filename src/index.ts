...
function rankTools(tools: McpTool[], prompt: string): RankedTool[] {
  const promptTokens = new Set(tokenize(prompt));

  return tools
    .map((tool) => {
      const haystack = tokenize(
        [tool.name, tool.description ?? '', schemaText(tool.inputSchema)].join(' ')
      );
      let score = 0;
...
function buildPlanPrompt(request: string, tools: McpTool[]): string {
...
function parseCliArgs(): { request: string; promptOnly: boolean } {
...
async function analyzeWithLocalLlm(request: string, tools: McpTool[]): Promise<LocalPlan | null> {
  const body = {
    ...
    messages: [
      {
        role: 'system',
        content:
          'You are a strict JSON tool planner. Never invent tools. Use only the provided tool catalog.',
      },
      {
        role: 'user',
  ...
  const parsed = JSON.parse(content.slice(jsonStart, jsonEnd + 1)) as LocalPlan;
